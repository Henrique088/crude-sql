...
class Funcoes():
    ...
    def criar_tabela(self):
        ...
        self.cursor.execute("""
                CREATE TABLE IF NOT EXISTS conta(
                    ...
                    agencia INTEGER(11) NOT NULL,
                    conta INTEGER(11) NOT NULL);""");
        self.conexao.commit();
    ...
    def deleta_cliente(self):
        self.capturar_campos()
        self.db_conect()
        self.cursor.execute("""DELETE FROM clientes WHERE id = ?""", [self.entry_codigo.get()])
        self.conexao.commit()
        self.db_desconect()
        self.limpar_campos()
        self.select_lista()
...
class Aplication(Funcoes):
    ...
    def tela(self):
        ...
        self.root.configure(background='#243625')
    ...
    def widgets_frame1(self):
        ...
        self.bt_limpar = Button(self.frame1, text="Limpar",
                                bg="#3e5e2b", fg="white", font=('verdana', 8, 'bold'), command=self.limpar_campos)
        self.bt_limpar.place(relx=0.2, rely=0.1, relwidth=0.1, relheight=0.15)

        # botão Buscar
        self.bt_buscar = Button(self.frame1, text="Buscar",
                                bg="#3e5e2b", fg="white", font=('verdana', 8, 'bold'), command=self.Buscar_Cliente)
        self.bt_buscar.place(relx=0.3, rely=0.1, relwidth=0.1, relheight=0.15)

        # botão Novo
        self.bt_novo = Button(self.frame1, text="Novo",
                              bg="#3e5e2b", fg="white", font=('verdana', 8, 'bold'), command=self.add_cliente)
        self.bt_novo.place(relx=0.6, rely=0.1, relwidth=0.1, relheight=0.15)

        # Botão Altera
        self.bt_alterar = Button(self.frame1, text="Alterar",
                                 bg="#3e5e2b", fg="white", font=('verdana', 8, 'bold'), command=self.alterar_cliente)
        self.bt_alterar.place(relx=0.7, rely=0.1, relwidth=0.1, relheight=0.15)

        # Botão Apagar
        self.bt_apagar = Button(self.frame1, text="Apagar",
                                bg="#571616", fg="white", font=('verdana', 8, 'bold'), command=self.deleta_cliente)
        self.bt_apagar.place(relx=0.8, rely=0.1, relwidth=0.1, relheight=0.15)

        # label e entry - codigo -----------------------------
        self.lb_codigo = Label(self.frame1, text="Codigo",
                               bg="white", fg="#182113", font=('verdana', 10, 'bold'))
